...
@cinema.get("/movies", tags=["movies"],description="We can display information regarding every movie present in the collection")
...
@cinema.post("/user/movie/watching", tags=["movie_watching"],description="Updating the user watched movie info if not created creating a new parameter and updating(pushing) into it")
async def movie_watched_info(*, token: str = Depends(oauth2_schema), Movie: str):
    if users.find({'movies_watched': {'$exists': True}}):
        users.find_one_and_update({'user_name': token}, {"$push": {'movies_watched': Movie}})
    else:
        users.find_one_and_update({'user_name': token}, {"$set": {'movies_watched': list[Movie]}})
    return serializelist(users.find())


@cinema.post("/user/movie/rating", tags=["Giving A Rating Value"],description="giving a rating to movie")
async def movie_rating(Movie: str, Rating: float):
    if not users.find_one({'movies_watched': Movie}):
